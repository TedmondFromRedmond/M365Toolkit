...
# Execute the environment setup with variables
# import functions.ps1 - Dot sourced from crt. dir
# invoke-pester with passthru
# Import pester.tests.ps1 - Test cases. Can be defined outside script and dot sourced from crt. dir.
# This script is a special case since it is a user interface test.
# A powershell window is opened that keystrokes are sent to.
# The window executes the function and the function returns an object with data.
# The object is piped to an output file that is checked for greater than a length of zero.
# If gt zero then the test was successful. 
# if ! gt zero, then an error is thrown.
# Tests start in the pester.Tests.ps1 area after importedTry catch with throw and should be used to put errors in the returned pester object in caller script
# Executes the tests
