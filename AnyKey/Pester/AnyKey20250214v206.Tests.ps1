...
#----
    ...
    Context 'Function behavior' {
        ...
        It 'Test Case 1 Writes the expected prompt and calls ReadKey' {
...
# Script to be tested (MyScript.ps1)
 function Get-UserInput {
   $name = Read-Host "Enter your name:"
   return "Hello, $name!"
 }

 # Pester test (MyScript.tests.ps1)
 Describe "Get-UserInput" {
   It "should return a greeting with mocked input" {
     Mock Read-Host { "Test User" } -ParameterFilter { $Prompt -like "Enter your name:*" }
     $result = Get-UserInput
     $result | Should -Be "Hello, Test User!"
   }
 }
<#
# Script to be tested (MyScript.ps1)
